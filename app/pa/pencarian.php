				...
				<div class="article">
				  <h2><span>mahasiswa</span></h2>
					...
					{ 
						...
						$limit=30;
						$dosenuser=$_SESSION[uid];
						$isidosen=mysql_fetch_array(mysql_query("select * from dosen where DosenNIK='$dosenuser'"));

		
						//query ke table mahasiswa diurutkan Desc berdasar MHSID
						$kunci=$_GET['kunci'];
							if($_GET['pilihcari']=="MHSNAMA" )
							{
							$numresult=@mysql_query("select * from mahasiswa where MHSPA='$isidosen[DosenId]' AND jurusanId='$_GET[JurusanId]' AND MHSNAMA LIKE '%$kunci%' order by MHSID ASC");
							}
							if($_GET['pilihcari']=="MHSID" )
							{
							$numresult=@mysql_query("select * from mahasiswa where MHSPA='$isidosen[DosenId]' AND jurusanId='$_GET[JurusanId]' AND MHSID LIKE '%$kunci%' order by MHSID ASC");
							}
							if($_GET['pilihcari']=="MHSKOTA" )
							{
							$numresult=@mysql_query("select * from mahasiswa where MHSPA='$isidosen[DosenId]' AND jurusanId='$_GET[JurusanId]' AND MHSKOTA LIKE '%$kunci%' order by MHSID ASC");
							}
						...
						else 
						{
							...
							echo "<p>Ditemukan <b>$numrow</b>  mahasiswa</p>";
							// ambil hasil
							$n= "0";
							$i="1";
							...
							if($_GET['pilihcari']=="MHSNAMA" )
							{
							$result=@mysql_query("select * from mahasiswa where MHSPA='$isidosen[DosenId]' AND jurusanId='$_GET[JurusanId]' AND MHSNAMA LIKE '%$kunci%' order by MHSID ASC  limit $offset,$limit");
							}
							if($_GET['pilihcari']=="MHSID" )
							{
							$result=@mysql_query("select * from mahasiswa where MHSPA='$isidosen[DosenId]' AND jurusanId='$_GET[JurusanId]' AND MHSID LIKE '%$kunci%' order by MHSID ASC  limit $offset,$limit");
							}
							if($_GET['pilihcari']=="MHSKOTA" )
							{
							$result=@mysql_query("select * from mahasiswa where MHSPA='$isidosen[DosenId]' AND jurusanId='$_GET[JurusanId]' AND MHSKOTA LIKE '%$kunci%' order by MHSID ASC  limit $offset,$limit");
							}
							
							echo "<table border=0 width='100%'>";
							echo "<tr bgcolor=#dedede><td colspan=12></td></tr>";
							echo "<tr bgcolor=#dedede><td colspan=12></td></tr>";
							echo "<tr bgcolor=grey><th >No.</th><th>NIM</th><th>NAMA</th><TH>JK</TH><TH>ALAMAT</TH><TH>KOTA</TH><TH>NO TELPON</TH><TH>ORTU</TH></tr>";
							echo "<tr bgcolor=#dedede><td colspan=12></td></tr>";
