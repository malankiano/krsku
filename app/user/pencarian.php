			...
			<div class="article">
			  <h2><span>Dosen</span></h2>
				...
				{ 
					...
					$kunci=$_GET['kunci'];
						if($_GET['pilihcari']=="nama" )
				  		{
						$numresult=@mysql_query("select * from user where nama LIKE '%$kunci%' order by nama ASC");
						}
						if($_GET['pilihcari']=="username" )
				  		{
						$numresult=@mysql_query("select * from user where  username LIKE '%$kunci%' order by nama ASC");
						}

	
					//menghitung  jumlah total seluruh staff pengelola 
					...
					//jika tidak ada staff pengelola
					if ($numrow==0) 
					{
					...
					else 
					{
						...
						//menmapilkan staff pengelola
						echo "<p>Ditemukan <b>$numrow</b>  Staff Pengelola</p>";
						// ambil hasil
						$n= "0";
						$i="1";
						...
						if($_GET['pilihcari']=="nama" )
				  		{
						$result=@mysql_query("select * from user where nama LIKE '%$kunci%' order by nama ASC  limit $offset,$limit");
						}
						if($_GET['pilihcari']=="username" )
				  		{
						$result=@mysql_query("select * from user where username LIKE '%$kunci%' order by nama ASC  limit $offset,$limit");
						}
